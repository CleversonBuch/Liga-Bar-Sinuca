...
export async function getRankings(period: 'mes' | 'trimestre' | 'semestre' | 'ano' = 'mes') {
    const supabase = await createClient()
    const now = new Date()
    const currentYear = now.getFullYear()
    const currentMonth = now.getMonth() + 1

    let monthsToFetch: number[] = [currentMonth]

    if (period === 'trimestre') {
        const quarter = Math.floor((currentMonth - 1) / 3)
        monthsToFetch = [quarter * 3 + 1, quarter * 3 + 2, quarter * 3 + 3]
    } else if (period === 'semestre') {
        const semester = Math.floor((currentMonth - 1) / 6)
        monthsToFetch = [semester * 6 + 1, semester * 6 + 2, semester * 6 + 3, semester * 6 + 4, semester * 6 + 5, semester * 6 + 6]
    } else if (period === 'ano') {
