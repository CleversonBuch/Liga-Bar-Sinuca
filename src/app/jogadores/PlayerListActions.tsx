...
export function PlayerListActions({ player, isSuperAdmin }: { player: any, isSuperAdmin: boolean }) {
    ...
    async function handleDelete() {
        if (!isSuperAdmin) {
            toast.error('Apenas o Administrador Mestre pode excluir jogadores.')
            return
        }

        if (!window.confirm('Tem certeza que deseja apagar este jogador? Todo o histórico dele será perdido.')) return
